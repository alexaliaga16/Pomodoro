  ...
  return (
    <div className="w-[min(100%,425px)] p-1.5">
      <div className="relative flex h-[120px] overflow-hidden rounded-3xl border border-white/20 shadow-[0_14px_50px_rgba(0,0,0,0.6)]">
        <div
          key={`${actual.name}-backdrop`}
          className="player-station-backdrop absolute inset-0 z-0 bg-cover bg-center transition-all duration-500"
          style={{
            backgroundImage: `linear-gradient(135deg, rgba(var(--accent-r), var(--accent-g), var(--accent-b), calc(0.34 * var(--accent-strength))) 0%, rgba(10,14,20,0.28) 45%, rgba(10,14,20,0.82) 100%), url(${actual.image || "/hero.png"})`,
          }}
        />
        ...
        <div
          ...
          style={{ boxShadow: `inset 0 0 50px rgba(var(--accent-r), var(--accent-g), var(--accent-b), calc(0.42 * var(--accent-strength)))` }}
        />

        <div className="relative z-10 flex flex-1 items-center gap-4 px-4 py-4 animate-[fadeSlide_.4s_ease]">
          <div
            key={`${actual.name}-art`}
            className="player-station-art h-[84px] w-[84px] shrink-0 overflow-hidden rounded-2xl border border-white/14 shadow-[0_12px_28px_rgba(0,0,0,0.28)]"
            style={{
              backgroundImage: `linear-gradient(135deg, rgba(var(--accent-r), var(--accent-g), var(--accent-b), calc(0.34 * var(--accent-strength))) 0%, rgba(10,14,20,0.18) 100%), url(${actual.image || "/hero.png"})`,
          ...
          <div className="flex flex-1 flex-col justify-between self-stretch">
            <div className="flex items-start justify-between gap-3">
              <div key={`${actual.name}-copy`} className="player-station-copy">
            ...
            <div className="flex items-center gap-2">
              <div className="relative h-[2px] flex-1 overflow-hidden rounded-full bg-white/10">
                <div className="absolute inset-0 transition-colors duration-500" style={{ background: `rgb(var(--accent-r), var(--accent-g), var(--accent-b))` }} />
                <div
                  className="absolute top-0 right-0 h-full bg-black animate-[liveEdge_2s_ease-in-out_infinite]"
                  style={{ animationPlayState: isPlaying ? "running" : "paused" }}
                />
              </div>
              <span className="rounded-full border px-2.5 py-0.5 text-[10px] uppercase tracking-[0.18em] text-white/70 transition-colors duration-500" style={{ borderColor: `rgba(var(--accent-r), var(--accent-g), var(--accent-b), 0.55)` }}>
                Live
              </span>
            </div>
