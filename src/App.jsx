...
function TareaItem({ tarea, cambiarEstado }) {
  ...
  return (
    ...
    >
      <div
        onClick={(e) => { e.stopPropagation(); cambiarEstado(tarea.id) }}
        className={`w-4 h-4 rounded-full cursor-pointer ${colorEstado(tarea.estado)} transition-all duration-300 hover:scale-125 active:scale-95`}
      />
      <span className={`flex-1 transition-all duration-300 ${tarea.estado === 'terminado' ? 'line-through text-white/40' : 'text-white/90'}`}>
        {tarea.texto}
      </span>
      <div {...attributes} {...listeners} className="cursor-grab text-white/40 hover:text-white">
        ☰
      </div>
    </div>
  )
...
function App() {
  ...
  const agregarTarea = () => {
    ...
    const nueva = { id: Date.now(), texto: nuevaTarea, estado: 'pendiente' }
  ...
  const cambiarEstado = (id) => {
    setTareas(prev =>
      prev.map(t =>
        t.id === id
          ? { ...t, estado: t.estado === 'pendiente' ? 'activo' : t.estado === 'activo' ? 'terminado' : 'pendiente' }
          : t
      )
    )
  }
  ...
  return (
    ...
    >
      ...
      {/* Panel izquierdo */}
      <div className={`absolute z-50 left-5 top-1/2 -translate-y-1/2 w-80 h-[calc(100vh-200px)]
        ...
        <h2 className="text-sm font-bold tracking-widest pl-2 mb-4">TAREAS</h2>

        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={tareas.map(t => t.id)} strategy={verticalListSortingStrategy}>
            <div className="flex-1 flex flex-col gap-3 overflow-y-auto pr-1">
              {tareas.map(t => (
                <TareaItem key={t.id} tarea={t} cambiarEstado={cambiarEstado} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
      ...
      {/* Panel derecho */}
      <div className={`absolute z-50 right-5 top-1/2 -translate-y-1/2 w-80 h-[calc(100vh-200px)]
        bg-black/60 backdrop-blur-sm rounded-3xl p-4 border border-white/10
        transition-all duration-500
        ${corriendo ? 'opacity-0 translate-x-8 pointer-events-none' : 'opacity-100 translate-x-0'}`}>

        <h2 className="text-sm font-bold tracking-widest pl-2">VIBES</h2>
